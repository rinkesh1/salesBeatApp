    ...
    void defineRegions(ArrayList<ArrayList<Region>> regions, ArrayList<ChartSet> data) {
        ...
        for (int i = 0; i < nEntries; i++) {
            ...
            for (int j = 0; j < nSets; j++) {
                ...
                if (bar.getValue() > 0 && (int) bar.getY() != axisMinValue)
                    regions.get(j).get(i).set((int) offset, (int) bar.getY(),
                            (int) (offset += barWidth), axisMinValue);
                else if (bar.getValue() < 0 && (int) bar.getY() != axisMinValue)
                    regions.get(j).get(i).set((int) offset, axisMinValue,
                            (int) (offset += barWidth), (int) bar.getY());
                else // If bar.getValue() == 0, force region to 1 pixel
                    regions.get(j).get(i).set((int) offset, axisMinValue,
                            (int) (offset += barWidth), axisMinValue + 1);
